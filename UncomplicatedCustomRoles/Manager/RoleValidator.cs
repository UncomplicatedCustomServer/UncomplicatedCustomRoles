...
internal static class RoleValidator
{
    ...
    private static readonly Regex PlaceholderRegex = new("%([A-Za-z_]+)%", RegexOptions.Compiled);
    ...
    private static string[] ResolveEffectNames()
    {
        ...
        {
            LogManager.Warn($"[Role Validator] Could not enumerate the game's status effects, 'effects' values won't be validated: {e.Message}");
            return [];
        }
    }
    ...
    {
        Validate(role, out List<string> errors, out _);
    ...
    {
        bool nameUsable = role.BadgeName is not null && role.BadgeName.Length > 1;
        bool colorUsable = role.BadgeColor is not null && role.BadgeColor.Length > 2;
        bool nameSet = !string.IsNullOrWhiteSpace(role.BadgeName);
        bool colorSet = !string.IsNullOrWhiteSpace(role.BadgeColor);

        if ((nameSet || colorSet) && (!nameUsable || !colorUsable))
        {
            if (nameSet && !nameUsable)
                warnings.Add($"'badge_name' ('{role.BadgeName}') is too short (at least 2 characters); the badge will not be applied.");
    ...
    private static void ValidateRoles(ICustomRole role, List<string> errors, List<string> warnings)
    {
        if (role.Role is RoleTypeId.None || role.Role.GetTeam() is Team.Dead)
            errors.Add($"'role' must be a valid role, got '{role.Role}'. Examples: ClassD, Scientist, NtfSergeant, Scp0492.");

        if (role.RoleAppearance is not RoleTypeId.None && role.RoleAppearance.GetTeam() is Team.Dead)
            warnings.Add($"'role_appearance' '{role.RoleAppearance}' is not a valid alive role; the role will keep the appearance of '{role.Role}'.");
    }
    ...
    private static void ValidateHealthLike(ICustomRole role, List<string> errors, List<string> warnings)
    {
        ...
        if (role.Ahp is not null)
        {
            ...
            if (role.Ahp.Efficacy is < 0f or > 1f)
                warnings.Add($"'ahp.efficacy' should be between 0 and 1 (fraction of damage absorbed), got {role.Ahp.Efficacy}.");
            if (role.Ahp.Decay < 0)
    ...
    private static void ValidateEffects(ICustomRole role, List<string> warnings)
    {
        ...
        for (int i = 0; i < role.Effects.Count; i++)
        {
            Effect effect = role.Effects[i];
            if (effect is null)
            {
            ...
            if (effect.Intensity == 0)
                warnings.Add($"'effects' entry #{i + 1} ('{effect.EffectType}') has intensity 0, which disables the effect; use at least 1.");
        }
    }
    ...
    private static void ValidateInventory(ICustomRole role, List<string> warnings)
    {
        if (role.Inventory is not null)
        {
            if (role.Inventory.Count > 8)
                warnings.Add($"'inventory' lists {role.Inventory.Count} items but a player only has 8 slots; the extra items will not fit.");

            foreach (ItemType item in role.Inventory.Where(IsAmmo))
                warnings.Add($"'inventory' contains the ammo '{item}'; put ammo under 'ammo:' instead so the amount is respected.");
    ...
    private static void ValidateMisc(ICustomRole role, List<string> warnings)
    {
        ...
        if (role.DamageMultiplier < 0)
            warnings.Add($"'damage_multiplier' is negative ({role.DamageMultiplier}); the role would heal targets instead of damaging them.");

        if (role.SpawnHintDuration < 0)
            warnings.Add($"'spawn_hint_duration' is negative ({role.SpawnHintDuration}).");

        Vector3 scale = role.Scale;
        ...
        foreach (KeyValuePair<string, Dictionary<string, object>> flag in flags)
        {
            ...
            try
            {
                ...
                if (!module.Validate(out string error))
                    LogManager.Warn($"[Role Validator] {label}: custom flag '{type.Name}' has an invalid setting: {error} It will be skipped on spawn.");
            }
            catch (Exception e)
            {
