...
public class Percentages : IUCRCommand
{
    ...
    public bool Executor(List<string> args, ICommandSender sender, out string response)
    {
        ...
        {
            var customRoles = CustomRole.List.Where(r =>
            ...
            foreach (var customRole in customRoles.OrderByDescending(r => r.SpawnSettings.SpawnChance))
            {
                var chance = customRole.SpawnSettings.SpawnChance;

                var actual = total <= 0 ? 0 : chance / Math.Max(total, 100) * 100;
        ...
        var delayedRoles = CustomRole.List.Where(r =>
        ...
        if (delayedRoles.Any())
        {
            ...
            foreach (var customRole in delayedRoles)
                response +=
        ...
        var manualRoles = CustomRole.List.Where(r =>
        ...
        if (manualRoles.Any())
        {
            ...
            foreach (var customRole in manualRoles)
