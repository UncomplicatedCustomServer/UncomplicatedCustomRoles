    ...
    public bool Executor(List<string> args, ICommandSender sender, out string response)
    {
        ...
        foreach (RoleTypeId role in Enum.GetValues(typeof(RoleTypeId)))
        {
            ...
            if (customRoles.Any())
            {
                ...
                response +=
                    $"\nChance of spawning as a <b>CustomRole</b>: {total}%\nChance of spawning as a regular role: {100 - total}%";

                if (detailed)
                    foreach (var customRole in customRoles.Where(r => r.SpawnSettings.SpawnChance > 0))
                        response += $"\n  ∟ {customRole} - {customRole.SpawnSettings.SpawnChance}%";
            }
        }

        var manualRoles = CustomRole.List.Where(r =>
            r.SpawnSettings?.CanReplaceRoles == null || !r.SpawnSettings.CanReplaceRoles.Any());
        if (manualRoles.Any())
        {
            response +=
