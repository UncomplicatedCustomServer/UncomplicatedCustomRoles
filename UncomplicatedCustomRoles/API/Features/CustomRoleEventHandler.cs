        ...
        try
        {
            if (Role is EventCustomRole customRoleEventsRole)
            {
                                 ...
                                 m.Name is not "OnSpawned"))
                {
                    ...
                    {
                        Listeners.Add(new Listener(derivedMethod.GetParameters()[0].ParameterType, derivedMethod,
                            customRoleEventsRole));
                        LogManager.Debug(
                            $"Loaded listener for [Event]CustomRole {customRoleEventsRole}: EVENT={derivedMethod.GetParameters()[0].ParameterType}, METHOD={derivedMethod.Name}()");
                    }
                }
            }
        }
