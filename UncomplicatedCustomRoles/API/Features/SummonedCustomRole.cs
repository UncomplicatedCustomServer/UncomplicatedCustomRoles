...
#pragma warning disable IDE1006 // Stili di denominazione
    public class SummonedCustomRole
    {
        ...
        private IEnumerator<float> RoleTickCoroutine()
        {
            ...
            {
                if (EvaluateCustomActions() && Player.ReferenceHub.playerStats.GetModule<HumeShieldStat>().CurValue < Role.Health.HumeShield && DateTimeOffset.UtcNow.ToUnixTimeSeconds() - LastDamageTime >= Role.Health.HumeShieldRegenerationDelay && !_isRegeneratingHume)
        ...
        /// </summary>
        /// <returns></returns>
        internal string ParseRemoteAdmin() => $"\n<size=26><color=#f55505>UncomplicatedCustomRoles</color></size>\nCustom Role: <color={(Role.Role.TryGetRoleBase(out PlayerRoleBase role) ? role.RoleColor.ToHex() : "white")}>{Role.Name}</color> [Id={Role.Id}]{LoadRoleFlags()}\n{LoadBadge()}";
        ...
        public IEnumerator<float> HumeShieldCoroutine()
        {
            ...
            while (_internalValid && Player.IsAlive && Player.ReferenceHub.playerStats.GetModule<HumeShieldStat>().CurValue < Role.Health.HumeShield && DateTimeOffset.UtcNow.ToUnixTimeSeconds() - LastDamageTime >= Role.Health.HumeShieldRegenerationDelay)
            {
                Player.ReferenceHub.playerStats.GetModule<HumeShieldStat>().CurValue += Role.Health.HumeShieldRegenerationAmount;
            ...
            foreach (SummonedCustomRole Role in List)
                if (Role.InfiniteEffects.Count() > 0)
                    foreach (IEffect Effect in Role.InfiniteEffects)
                        if (!Role.Player.ReferenceHub.playerEffectsController.AllEffects.Any(e => e.name == Effect.EffectName))
                            Role.Player.ReferenceHub.playerEffectsController.ChangeState(Effect.EffectName, Effect.Intensity, float.MaxValue, false);
        }
    }
}
