...
public class SpawnPoint
{
    ...
    /// <summary>
    ///     Gets whether the <see cref="SpawnPoint" /> is fixed in the position or not (if fixed then it's NOT linked to the
    ///     room)
    /// </summary>
    [JsonIgnore]
    ...
    {
        return CorrectLocation(Room);
    }

    private Vector3 CorrectLocation(Room room)
    {
        if (Fixed || room is null)
            return Position;
    ...
    /// </summary>
    /// <remarks>
    ///     If a room is available, the player's position is set relative to the room's position;
    ///     otherwise, the player's position is set to the spawn point's position. In both cases, the player's rotation
    ///     is set to match the spawn point.
    /// </remarks>
    ...
    /// <returns>
    ///     A <see cref="SpawnPoint" /> instance configured at the specified position and rotation, with fixed
    ...
    {
        return List.FirstOrDefault(sp => sp.Name == name) ?? UnsyncedList.FirstOrDefault(sp => sp.Name == name);
    }
