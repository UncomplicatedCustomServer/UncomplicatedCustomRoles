...
public class CustomKeycard : CustomModule
{
    ...
    public override void OnAdded()
    {
        Timing.CallDelayed(Timing.WaitForOneFrame, () =>
        {
            ...
            {
                LogManager.Error($"[CustomKeycard] The provided keycard type ({KeycardType}) is not customizable or does not exist.");
                return;
            }
            ...
            if (_keycardItem is null)
                LogManager.Error($"[CustomKeycard] Failed to create keycard of type {KeycardType} for player {Player?.Nickname}.");
        });
        base.OnAdded();
    }
    ...
    private T ParseEnum<T>(string param, T def) where T : struct, Enum
    {
        ...
        if (raw is null)
            return def;
        return Enum.TryParse(raw, true, out T result) ? result : def;
    }
    ...
    private Color ParseColor(string param, Color def)
    {
        ...
        if (!raw.StartsWith("#"))
            raw = "#" + raw;
        return ColorUtility.TryParseHtmlString(raw, out Color color) ? color : def;
    }
}
